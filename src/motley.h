...
void UpdateWindow(Window *state) {
  if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
      CheckCollisionPointRec(GetMousePosition(),
                             (Rectangle){state->Anchor.x, state->Anchor.y,
                                         state->Size.x,
  ...
  bool xPressed = GuiWindowBox((Rectangle){state->Anchor.x, state->Anchor.y,
...
KeybindMenuState NewKeybindMenu() {
  ...
  state.layoutRecs[0] =
      (Rectangle){state.anchor01.x + 0, state.anchor01.y + 0, 264, 264};
  state.layoutRecs[1] =
      (Rectangle){state.anchor01.x + 32, state.anchor01.y + 24, 120, 24};
  state.layoutRecs[2] =
      (Rectangle){state.anchor01.x + 104, state.anchor01.y + 24, 48, 24};
...
void KeybindMenu(KeybindMenuState *state) {
  GuiScrollPanel(
      (Rectangle){state->layoutRecs[0].x, state->layoutRecs[0].y,
                  state->layoutRecs[0].width - state->ScrollPanelBoundsOffset.x,
                  state->layoutRecs[0].height -
                      state->ScrollPanelBoundsOffset.y},
  ...
  for (u32 i = 0; i < ACTION_COUNT * MAX_KEYBINDS; i += MAX_KEYBINDS) {
    ...
    GuiLabel((Rectangle){.x = 0, .y = 0 + i * 24, .width = 48, .height = 24},
             buf);

    state->RebindButtons[i] =
        GuiButton((Rectangle){.x = state->layoutRecs[2].x,
                              .y = state->layoutRecs[2].y + i * 8,
                              .width = 48,
                              .height = 24},
                  buf);
    state->RebindButtons[i + 1] =
        GuiButton((Rectangle){.x = state->layoutRecs[2].x + 24,
                              .y = state->layoutRecs[2].y + i * 8,
                              .width = 48,
                              .height = 24},
                  buf);
    state->RebindButtons[i + 2] =
        GuiButton((Rectangle){.x = state->layoutRecs[2].x + 48,
                              .y = state->layoutRecs[2].y + i * 8,
                              .width = 48,
                              .height = 24},
                  buf);
    state->RebindButtons[i + 3] =
        GuiButton((Rectangle){.x = state->layoutRecs[2].x + 72,
                              .y = state->layoutRecs[2].y + i * 8,
                              .width = 48,
                              .height = 24},
                  buf);
...
v2u GetHoveredTile(const Tilemap *map) {
  v2 tAnchor = Vector2Add(map->Window.Anchor,
                          (v2){0, RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT});
  return (v2u){(GetMouseX() - tAnchor.x) / map->tileSize.x,
               (GetMouseY() - tAnchor.y) / map->tileSize.y};
}

void DrawTileCursor(const Tilemap *map, const TilemapCursor *cursor,
                    const ToolbarState *toolbar, Shader shaders[2]) {
  ...
  v2 selected = toolbar->PaintTileActive
                    ? (v2){map->tileSize.x * cursor->Selected.x,
...
void CopyBoxedTiles(const Tilemap *map, const TilemapCursor *cursor) {
  if (CopyData.Data[0].Tile)
    free(CopyData.Data[0].Tile);
  if (CopyData.Data[0].FG)
    free(CopyData.Data[0].FG);
  if (CopyData.Data[0].BG)
    free(CopyData.Data[0].BG);
  if (CopyData.Data[0].Rot)
    free(CopyData.Data[0].Rot);
  ...
  v2u endBox = (v2u){
      cursor->Box[0].x >= cursor->Box[1].x ? cursor->Box[0].x
                                           : cursor->Box[1].x,
      cursor->Box[0].y >= cursor->Box[1].y ? cursor->Box[0].y
                                           : cursor->Box[1].y,
  };
...
void DeleteBoxedTiles(Tilemap *map, const TilemapCursor *cursor) {
  v2u from = {cursor->Box[0].x < cursor->Box[1].x ? cursor->Box[0].x
                                                  : cursor->Box[1].x,
              cursor->Box[0].y < cursor->Box[1].y ? cursor->Box[0].y
                                                  : cursor->Box[1].y};
  v2u to = {cursor->Box[0].x >= cursor->Box[1].x ? cursor->Box[0].x
                                                 : cursor->Box[1].x,
            cursor->Box[0].y >= cursor->Box[1].y ? cursor->Box[0].y
                                                 : cursor->Box[1].y};
...
void PasteBoxedTiles(Tilemap *map, const TilemapCursor *cursor,
  ...
  for (u32 x = 0; x < CopyData.Len.x; x++) {
    for (u32 y = 0; y < CopyData.Len.y; y++) {
      if (hoveredTile.y + y >= map->Size.y)
        continue;
      if (hoveredTile.x + x >= map->Size.x)
        continue;
...
void SetFgColor(u8 id, TilemapCursor *cursor, const Tilemap *map,
                ToolbarState *toolbar) {
  u8 newFg = id % map->PaletteSize;
  cursor->FG = newFg != cursor->BG ? newFg : ((newFg + 1) % map->PaletteSize);
}

void SetBgColor(u8 id, TilemapCursor *cursor, const Tilemap *map,
                ToolbarState *toolbar) {
...
void UpdateTileCursor(Tilemap *map, TilemapCursor *cursor,
                      ToolbarState *toolbar) {
  if (!map->Window.Active || GlobalGuiState.Editing || GlobalGuiState.Moving)
    return;
  ...
  cursor->InMap = CheckCollisionPointRec(
      mousePos, (Rectangle){map->Window.Anchor.x, map->Window.Anchor.y + 24,
                            map->Size.x * map->tileSize.x,
                            map->Size.y * map->tileSize.y});

  v2 absPos = Vector2Subtract(
      mousePos, Vector2Add(map->Window.Anchor,
                           (v2){0, RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT}));
  ...
  if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && cursor->InMap) {
    ...
    if (toolbar->PaintTileActive)
      map->Layer[map->SelectedLayer].Tile[i] =
          (v2u){cursor->Selected.x, cursor->Selected.y};
    if (toolbar->PaintFgActive)
      map->Layer[map->SelectedLayer].FG[i] = cursor->FG;
    if (toolbar->PaintBgActive)
      map->Layer[map->SelectedLayer].BG[i] = cursor->BG;
    if (toolbar->PaintRotActive)
      map->Layer[map->SelectedLayer].Rot[i] = cursor->Rot;
  }
  ...
  if (IsActionPressed(ACTION_LEFT) && cursor->Selected.x != 0)
    cursor->Selected.x -= 1;
  if (IsActionPressed(ACTION_RIGHT) &&
      cursor->Selected.x < map->Tileset.width / map->tileSize.x - 1)
    cursor->Selected.x += 1;
  if (IsActionPressed(ACTION_UP) && cursor->Selected.y != 0)
    cursor->Selected.y -= 1;
  ...
  if (IsKeyPressed(KEY_R))
    cursor->Rot = (cursor->Rot + 1) % 4;
  if (IsKeyPressed(KEY_F))
    SetFgColor(cursor->FG + 1, cursor, map, toolbar);
  if (IsKeyPressed(KEY_B))
    SetBgColor(cursor->BG + 1, cursor, map, toolbar);
...
void UpdateBoxSelect(Tilemap *mapFrom, Tilemap *mapTo, TilemapCursor *cursor,
  ...
  v2 tAnchor = Vector2Add(mapFrom->Window.Anchor,
                          (v2){0, RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT});
  ...
  if (cursor->State == CURSOR_STATE_BOXED &&
...
void DrawBoxCursor(const Tilemap *map, const TilemapCursor *cursor) {
  if (cursor->State == CURSOR_STATE_SINGLE)
    return;
  ...
  DrawRectangleLinesEx(
      (Rectangle){start.x < end.x ? start.x : end.x,
...
bool DrawPopupLoad(PopupLoadState *state) {
  ...
  if (GuiTextBox((Rectangle){state->Window.Anchor.x + 48,
                             state->Window.Anchor.y + 32, 104, 24},
                 state->TilemapPathTextBoxText, 128,
                 state->TilemapPathTextBoxEditMode))
    state->TilemapPathTextBoxEditMode = !state->TilemapPathTextBoxEditMode;
  GuiLabel((Rectangle){state->Window.Anchor.x + 8, state->Window.Anchor.y + 32,
                       32, 24},
           TilemapPathLabelText);

  if (GuiButton((Rectangle){state->Window.Anchor.x + 24,
                            state->Window.Anchor.y + 72, 120, 24},
                LoadButtonText)) {
...
Tilemap InitTilemap(const PopupNewState *state, Arena *arena) {
  ...
  map.Window =
      InitWindowBox("*New tilemap*", (v2){map.Size.x * map.tileSize.x,
                                          map.Size.y * map.tileSize.y});
...
bool SaveTilemap(Tilemap *this, const char *path) {
  if (GlobalGuiState.Editing)
    return false;
  if (!this->Window.Active || !(IsActionPressed(ACTION_SAVE)))
    return false;
  ...
  for (u32 j = 0; j < MAX_LAYERS; j++) {
    for (u32 i = 0; i < this->Size.x * this->Size.y; i++) {
      fprintf(file, "%u %u ", this->Layer[j].Tile[i].x,
              this->Layer[j].Tile[i].y);
    }
    fprintf(file, "\n");
  ...
  for (u32 i = 0; i < this->PaletteSize; i++) {
    fprintf(file, "%d %d %d %d ", this->Palette[i].r, this->Palette[i].g,
...
Tilemap LoadTilemap(PopupLoadState *state, Arena *arena) {
  ...
  for (u32 j = 0; j < MAX_LAYERS; j++) {
    ...
    for (u32 i = 0; i < tileCount; i++)
      fscanf(file, "%hhu", &this.Layer[j].FG[i]);

    for (u32 i = 0; i < tileCount; i++)
      fscanf(file, "%hhu", &this.Layer[j].BG[i]);

    for (u32 i = 0; i < tileCount; i++)
      fscanf(file, "%hhu", &this.Layer[j].Rot[i]);
  }
  ...
  this.Window = InitWindowBox(
      state->TilemapPathTextBoxText,
    ...
    for (u32 y = 0; y < map->Size.y; y++) {
      ...
      DrawTexturePro(map->Tileset,
                     (Rectangle){map->tileSize.x * map->Layer[0].Tile[coord].x,
                                 map->tileSize.y * map->Layer[0].Tile[coord].y,
                                 map->tileSize.x, map->tileSize.y},
                     (Rectangle){map->tileSize.x * x + tAnchor.x,
                                 map->tileSize.y * y + tAnchor.y,
                                 map->tileSize.x, map->tileSize.y},
                     (v2){map->tileSize.x * 0.5f, map->tileSize.y * 0.5f},
                     map->Layer[0].Rot[coord] * 90.0f,
                     map->Palette[map->Layer[0].BG[coord]]);
    }
  }
  EndShaderMode();
      ...
      for (u32 y = 0; y < map->Size.y; y++) {
        ...
        DrawTexturePro(
            ...
            (Rectangle){map->tileSize.x * map->Layer[i].Tile[coord].x,
                        map->tileSize.y * map->Layer[i].Tile[coord].y,
                        map->tileSize.x, map->tileSize.y},
            (Rectangle){map->tileSize.x * x + tAnchor.x,
                        map->tileSize.y * y + tAnchor.y, map->tileSize.x,
                        map->tileSize.y},
            (v2){map->tileSize.x * 0.5f, map->tileSize.y * 0.5f},
            map->Layer[i].Rot[coord] * 90.0f,
            map->Palette[map->Layer[i].FG[coord]]);
      }
    };
  EndShaderMode();
}
...
PopupNewState InitNewTilemapMenu() {
  PopupNewState state = {.Window = {.Active = false,
                                    .Anchor = {GetScreenWidth() * 0.5f - 140,
                                               GetScreenHeight() * 0.5f - 76},
                                    .Title = "New tilemap",
                                    .Size = {272, 144}}};
...
bool DrawNewTilemapMenu(PopupNewState *state) {
  ...
  if (GuiTextBox((Rectangle){tAnchor.x + 8, tAnchor.y + 32, 120, 24},
  ...
  if (GuiTextBox((Rectangle){tAnchor.x + 8, tAnchor.y + 72, 120, 24},
  ...
  GuiLabel((Rectangle){tAnchor.x + 136, tAnchor.y + 32, 48, 24},
           tileSizeLabelText);
  GuiLabel((Rectangle){tAnchor.x + 136, tAnchor.y + 72, 56, 24},
           mapSizeLabelText);

  bool newTilemap =
      GuiButton((Rectangle){tAnchor.x + 72, tAnchor.y + 112, 120, 24},
...
void UpdateToolbar(ToolbarState *state) {
  ...
  if (IsKeyDown(KEY_LEFT_CONTROL) && IsKeyPressed(KEY_T))
    state->Window.Active = true;

  if (IsActionPressed(ACTION_PAINT_TILE))
    state->PaintTileActive = !state->PaintTileActive;
  if (IsActionPressed(ACTION_PAINT_FG))
    state->PaintFgActive = !state->PaintFgActive;
  if (IsActionPressed(ACTION_PAINT_BG))
    state->PaintBgActive = !state->PaintBgActive;
  if (IsActionPressed(ACTION_PAINT_ROT))
    state->PaintRotActive = !state->PaintRotActive;
}

void DrawToolbar(ToolbarState *state, PopupNewState *newTilemapMenuState,
                 PopupLoadState *popupLoadState, Tilemap *tilemap,
                 TilemapCursor *cursor) {
  ...
  GuiLine((Rectangle){state->Window.Anchor.x + 12, state->Window.Anchor.y + 56,
                      116, 16},
          NULL);

  if (GuiButton((Rectangle){state->Window.Anchor.x + 8,
                            state->Window.Anchor.y + 32, 56, 24},
                NewButtonText))
    newTilemapMenuState->Window.Active = true;

  if (GuiButton((Rectangle){state->Window.Anchor.x + 72,
                            state->Window.Anchor.y + 32, 56, 24},
                LoadButtonText))
    popupLoadState->Window.Active = true;

  GuiToggle((Rectangle){state->Window.Anchor.x + 8, state->Window.Anchor.y + 72,
                        24, 24},
            PaintTileText, &state->PaintTileActive);
  GuiToggle((Rectangle){state->Window.Anchor.x + 40,
                        state->Window.Anchor.y + 72, 24, 24},
            PaintFgText, &state->PaintFgActive);
  GuiToggle((Rectangle){state->Window.Anchor.x + 72,
                        state->Window.Anchor.y + 72, 24, 24},
            PaintBgText, &state->PaintBgActive);
  GuiToggle((Rectangle){state->Window.Anchor.x + 104,
                        state->Window.Anchor.y + 72, 24, 24},
  ...
  for (u32 y = 0; y < 4; y++) {
    for (u32 x = 0; x < 4; x++) {
      ...
      if (cursor->FG == i)
        DrawRectangleLinesEx((Rectangle){r.x, r.y, r.width, r.height}, 3,
                             GREEN);
      if (cursor->BG == i)
        DrawRectangleLinesEx((Rectangle){r.x, r.y, r.width, r.height}, 3,
                             YELLOW);

      if (i++ >= tilemap->PaletteSize)
        goto OUTER_BREAK;
    }
  }
OUTER_BREAK:
...
TilesetState InitTilesetWindow(v2 pos) {
  return (TilesetState){
      .Scale = 0.5f,
      .Window = {
          .Active = true, .Anchor = pos, .Title = "Tileset", .Size = {}}};
}

void UpdateTileset(TilesetState *state, const Tilemap *map) {
  if (!state->Window.Active || !map->Window.Active)
    return;

  // 24 = mrmo-x3 size
  state->Scale = map->tileSize.x >= 24 ? 0.5f : 1.0f;

  state->Window.Size = (v2){map->Tileset.width * state->Scale,
...
void DrawTileset(TilesetState *state, const Tilemap *map, TilemapCursor *cursor,
  ...
  v2 tAnchor = Vector2Add(state->Window.Anchor,
                          (v2){0, RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT});

  BeginShaderMode(shaders[0]);
  DrawTextureEx(map->Tileset, tAnchor, 0.0f, state->Scale,
  ...
  v2u scaledTile =
      (v2u){map->tileSize.x * state->Scale, map->tileSize.y * state->Scale};

  DrawRectangleLines((u32)(cursor->Selected.x * scaledTile.x) + tAnchor.x + 1,
                     (u32)(cursor->Selected.y * scaledTile.y) + tAnchor.y + 1,
                     scaledTile.x, scaledTile.y, GREEN);

  if (CheckCollisionPointRec(GetMousePosition(),
                             (Rectangle){tAnchor.x, tAnchor.y,
                                         map->Tileset.width * state->Scale,
                                         map->Tileset.height * state->Scale})) {
    ...
    DrawRectangleLines(hoveredTile.x * scaledTile.x + tAnchor.x + 1,
                       hoveredTile.y * scaledTile.y + tAnchor.y + 1,
                       scaledTile.x, scaledTile.y, BLUE);
...
Image ExportTilemap(const Tilemap *map) {
  Image result = GenImageColor(map->Size.x * map->tileSize.x,
                               map->Size.y * map->tileSize.y, BLANK);
  Image tileset = LoadImageFromTexture(map->Tileset);

  for (u32 y = 0; y < result.height; y++) {
    for (u32 x = 0; x < result.width; x++) {
      u32 mapCoord =
          (y / map->tileSize.y) * map->Size.x + (x / map->tileSize.x);
      ...
      case ROT_RIGHT: // (y, m - 1 - x)
        tilesetPixelOffset = (v2u){tilesetPixelOffset.y,
                                   map->tileSize.x - 1 - tilesetPixelOffset.x};
      ...
      case ROT_LEFT: // (n - 1 - y, x)
        tilesetPixelOffset = (v2u){map->tileSize.y - 1 - tilesetPixelOffset.y,
                                   tilesetPixelOffset.x};
        break;
      }

      v2u tilesetPixel = {map->Layer[0].Tile[mapCoord].x * map->tileSize.x +
                              tilesetPixelOffset.x,
                          map->Layer[0].Tile[mapCoord].y * map->tileSize.y +
                              tilesetPixelOffset.y};
      ...
      for (u32 i = 0; i < 1 /* MAX_LAYERS */; i++) {
        ...
        if (color.r == 0) {
          if (bgColor != 0)
            ImageDrawPixel(&result, x, y, map->Palette[bgColor]);
        } else {
          if (fgColor != 0)
            ImageDrawPixel(&result, x, y, map->Palette[fgColor]);
        }
      }
    }
  }
...
void SetBackgroundColor(Motley *Motley, v3 fg, v3 bg) {
  SetShaderValue(Motley->shaders[4],
                 GetShaderLocation(Motley->shaders[4], "color1"), &fg,
                 SHADER_UNIFORM_VEC3);
  SetShaderValue(Motley->shaders[4],
                 GetShaderLocation(Motley->shaders[4], "color2"), &bg,
                 SHADER_UNIFORM_VEC3);
}
...
void InitMotley() {
  M = (Motley){
      .toolbar = InitToolbar(
          (v2){GetScreenWidth() * 0.025f, GetScreenHeight() * 0.05f}),
      .tileset = InitTilesetWindow(
          (v2){GetScreenWidth() * 0.025f, GetScreenHeight() * 0.6f}),
      .cursor =
          (TilemapCursor){.FG = 2, .BG = 0, .CopyArena = InitArena(250 << 10)},
      ...
      .shaders = {
          ...
          LoadShader("src/shaders/tiles.vert",
                     "src/shaders/tile_fg_cursor.frag"),
          LoadShader("src/shaders/tiles.vert", "src/shaders/tile_bg.frag"),
          LoadShader("src/shaders/tiles.vert",
                     "src/shaders/tile_bg_cursor.frag"),
...
void UpdateMotley() {
  ...
  if (IsActionPressed(ACTION_EXPORT))
    ExportImage(ExportTilemap(&M.tilemap[0]), "tilemap.png");
  ...
  DrawTileset(&M.tileset, M.tilemap, &M.cursor,
              (Shader[2]){M.shaders[0], M.shaders[2]});
