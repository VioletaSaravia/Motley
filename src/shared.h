...
void f32dUpdate(f32d *this, f32 x) {
  ...
  f32 k2_stable =
      MAX(this->_k2, 1.1f * (delta * delta / 4 + delta * this->_k1 / 2));
  this->_yd = this->_yd +
              delta * (x + this->_k3 * xd - this->_y - this->_k1 * this->_yd) /
                  k2_stable;
}
...
v2 CartesianToPolar(v2 coords) {
  return (v2){sqrtf(pow(coords.x, 2) + pow(coords.y, 2)),
              atanf(coords.y / coords.x)};
}

v3 CalculateOrbit3D(f32 radius, f32 yaw, f32 pitch) {
  v3 ray = {0, 0, 1};

  ray = Vector3RotateByQuaternion(
      ray, QuaternionFromAxisAngle((v3){0, 1, 0}, TAU - yaw));

  v3 pitchAxis = Vector3CrossProduct(ray, (v3){0, 1, 0});
  ray = Vector3RotateByQuaternion(
      ray, QuaternionFromAxisAngle(pitchAxis, TAU - pitch));
...
Rectangle CheckOverlapRecs(Rectangle r1, Rectangle r2) {
  ...
  return (x > width || y > height) ? (Rectangle){}
...
#define AALLOC(arena, type, size)                                              \
  ((type *)ArenaAlloc(arena, sizeof(type) * size))
...
bool IsActionPressed(Action action) {
  for (u32 i = 0; i < MAX_KEYBINDS; i++) {
    ...
    for (u32 j = 0; j < MAX_MODS; j++) {
      if (key.Mods[j] != 0 && !IsKeyDown(key.Mods[j]))
        goto OUTER_CONTINUE;
    }
    ...
    case INPUT_TYPE_Mouse:
      if (IsMouseButtonPressed(key.Key))
        return true;
      break;
    default:
      if (IsGamepadButtonPressed(key.Type - 2, key.Key))
        return true;
      break;
    }
...
bool IsActionPressedRepeat(Action action) {
  for (int i = 0; i < MAX_KEYBINDS; i++) {
    ...
    for (u32 j = 0; j < MAX_MODS; j++) {
      if (!IsKeyDown(key.Mods[j]))
        continue;
    }

    switch (key.Type) {
    case INPUT_TYPE_Keyboard:
      if (IsKeyPressedRepeat(key.Key))
        return true;
      break;
    case INPUT_TYPE_Mouse:
      if (IsMouseButtonPressed(key.Key))
...
bool IsActionReleased(Action action) {
  for (int i = 0; i < MAX_KEYBINDS; i++) {
    ...
    case INPUT_TYPE_Keyboard:
      ...
      break;
    case INPUT_TYPE_Mouse:
      if (IsMouseButtonReleased(key.Key))
        return true;
      break;
    default:
      if (IsGamepadButtonReleased(key.Type - 2, key.Key))
        return true;
      break;
    }
  }
...
bool IsActionUp(Action action) {
  for (int i = 0; i < MAX_KEYBINDS; i++) {
    ...
    case INPUT_TYPE_Keyboard:
      ...
      break;
    case INPUT_TYPE_Mouse:
      if (IsMouseButtonUp(key.Key))
        return true;
      break;
    default:
      if (IsGamepadButtonUp(key.Type - 2, key.Key))
        return true;
      break;
    }
  }
...
bool IsActionDown(Action action) {
  for (int i = 0; i < MAX_KEYBINDS; i++) {
    ...
    case INPUT_TYPE_Keyboard:
      ...
      break;
    case INPUT_TYPE_Mouse:
      if (IsMouseButtonDown(key.Key))
        return true;
      break;
    default:
      if (IsGamepadButtonDown(key.Type - 2, key.Key))
        return true;
      break;
    }
  }
